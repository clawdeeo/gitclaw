...
impl Config {
    ...
    fn load_from_local() -> Result<Option<Self>> {
        ...
        if path.exists() {
            let content = fs::read_to_string(&path)
                .with_context(|| "Failed to read project-local config")?;
            let config: Config = toml::from_str(&content)
                .with_context(|| "Failed to parse project-local config")?;
            return Ok(Some(config));
        }
        Ok(None)
    }
    ...
    fn load_from_xdg() -> Result<Option<Self>> {
        if let Some(config_dir) = dirs::config_dir() {
            ...
            if path.exists() {
                let content = fs::read_to_string(&path)
                    .with_context(|| "Failed to read XDG config")?;
                let config: Config = toml::from_str(&content)
                    .with_context(|| "Failed to parse XDG config")?;
                return Ok(Some(config));
            }
        }
        Ok(None)
    ...
    fn load_from_legacy() -> Result<Option<Self>> {
        if let Some(home) = dirs::home_dir() {
            ...
            if path.exists() {
                let content = fs::read_to_string(&path)
                    .with_context(|| "Failed to read legacy config")?;
                let config: Config = toml::from_str(&content)
